...
struct connection * connection_create(int socket, struct msg_store *store)
{
...
struct msg_store * connection_get_msg_store(struct connection *conn)
{
...
struct client_state * connection_get_client_state(struct connection *conn)
{
  return &conn->client;
}
...
bool connection_receive(struct connection *conn,
                        char *buffer,
                        int buffer_size)
{
...
void connection_dispose(struct connection *conn)
{
  msg_store_dispose_client(conn->msg_store, &conn->client);
  fclose(conn->infile);
  fclose(conn->outfile);
  free(conn);
}
